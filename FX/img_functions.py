...
#To ignore a small amount of green things in the environment, remove some of the largest and smallest data
#c: the array to process; rate: percentage of data to be removed
def pre_process_c(c,rate):
    rate=rate/2 
    c_num=c.shape[0]
    point_to_delete=int(c_num*rate)
    c=heapq.nlargest(c_num-point_to_delete,c)
    c=heapq.nsmallest(c_num-2*point_to_delete,c)
    return c

def img_process(img,axs,pause_time=1):
    ...
    if fld_segments is None:
        ...
        if fld_segments.shape[0]>1:
            ...
            distribution_c=pre_process_c(distribution_c,0.1) #0.1 means delete 10% abnormal values in the array
